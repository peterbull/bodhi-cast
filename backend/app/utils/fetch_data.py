...
def parse_swell_data(json_data):
    hourly_data = json_data.requests.get('hourly', {})
    times = hourly_data.requests.get('time', [])
    heights = hourly_data.requests.get('swell_wave_height', [])
    periods = hourly_data.requests.get('swell_wave_period', [])

    swell_data_list = []

    for time, height, period in zip(times, heights, periods):
        swell_data = SwellData(
                    latitude=json_data.requests.get('latitude'),
                    longitude=json_data.requests.get('longitude'),
                    generationtime_ms=json_data.requests.get('generationtime_ms'),
                    utc_offset_seconds=json_data.requests.get('utc_offset_seconds'),
                    timezone=json_data.requests.get('timezone'),
                    timezone_abbreviation=json_data.requests.get('timezone_abbreviation'),
                    elevation=json_data.requests.get('elevation'),
                    hourly_units=json_data.requests.get('hourly_units'),
                    hourly=time,
